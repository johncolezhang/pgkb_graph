...
list_url_template = "https://www.yaopinnet.com/zhongyao1/{}.htm"
...
df_menu.to_csv("processed/chn_drug_description_list.csv", index=False)

df_menu = pd.read_csv("processed/chn_drug_description_list.csv", dtype=str).fillna("")
...
for index, row in tqdm(df_menu.iterrows()):
    ...
    if index != 0 and index % 1000 == 0:
        ...
        df_detail.to_csv("processed/chn_drug_description_detail_{}.csv".format(index), index=False)
        drug_description_dict = {}
...
if len(drug_description_dict.items()) > 0:
    ...
    df_detail = pd.DataFrame(description_dict)
    df_detail.to_csv("processed/chn_drug_description_detail_{}.csv".format(len(df_menu)), index=False)
